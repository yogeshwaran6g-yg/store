...
export default function DataModal({
  ...
  return (
    <Modal
      ...
      className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full mx-auto mt-20 outline-none overflow-hidden"
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-purple-50">
        ...
        <button 
          onClick={onClose}
          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/80 transition-colors group"
      ...
      {/* Content */}
      <div className="p-6 max-h-[65vh] overflow-auto bg-gray-50">
        {data ? (
          isJson ? (
            <div className="relative">
              <div className="absolute top-2 right-2 text-xs font-mono text-gray-400 bg-white px-2 py-1 rounded">
                JSON
              </div>
              <pre className="text-sm bg-white p-5 rounded-xl text-gray-800 whitespace-pre-wrap shadow-sm border border-gray-200 font-mono leading-relaxed">
                {JSON.stringify(data, null, 2)}
              </pre>
            </div>
          ) : (
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
              <p className="text-sm text-gray-800 break-words leading-relaxed">{data}</p>
            </div>
          )
        ) : (
